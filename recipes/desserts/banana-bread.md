---
title: Banana Bread
description: >-
  Those overripe Costco bananas sitting on your counter aren't a failure —
  they're an opportunity. The browner, the better.
prep: 15 min
cook: 55 min
servings: '10'
cost: 0.75
cost_unit: serving
category: desserts
vegetarian: true
gluten_free: false
dairy_free: false
costco_ingredients:
  - >-
    Costco Bananas — 4 large, very ripe (the blacker the better) (~$0.60, 3 lb
    bunch)
  - 'Kirkland All-Purpose Flour — 1 3/4 cups (~$0.30, 12 lb bag)'
  - 'Kirkland Unsalted Butter — 1/2 cup (1 stick), melted (~$0.50, 4-stick pack)'
  - 'Kirkland Large Eggs — 2 (~$0.50, 24-count)'
  - 'Kirkland Semi-Sweet Chocolate Chips — 1/2 cup, optional (~$0.75, 4.5 lb bag)'
other_ingredients:
  - 3/4 cup granulated sugar
  - 1 tsp pure vanilla extract
  - 1 tsp baking soda
  - 1/2 tsp salt
  - 1/2 tsp ground cinnamon
  - 1/4 cup sour cream or Greek yogurt
instructions:
  - >-
    Preheat the oven to 350 degrees. Grease a 9x5-inch loaf pan or line it with
    parchment paper, leaving some overhang for easy removal.
  - >-
    Mash the bananas in a large bowl with a fork. You want them mostly smooth
    with a few chunks for texture. If your bananas aren't brown and spotted,
    they're not ready — put this recipe on hold and wait a couple days.
  - >-
    Stir the melted butter into the mashed bananas. Add the sugar, eggs,
    vanilla, and sour cream. Mix until just combined.
  - >-
    In a separate bowl, whisk together the flour, baking soda, salt, and
    cinnamon.
  - >-
    Fold the dry ingredients into the wet ingredients using a spatula. Mix until
    the flour just disappears — overmixing is the enemy of tender banana bread.
    A few small lumps are perfectly fine.
  - Fold in the chocolate chips if using. (You should be using them.)
  - >-
    Pour the batter into the prepared loaf pan and smooth the top. For that
    bakery-style split down the middle, run a butter knife lengthwise through
    the center of the batter.
  - >-
    Bake for 50–55 minutes, until a toothpick inserted in the center comes out
    with just a few moist crumbs. The top should be deeply golden and the edges
    pulling away from the pan.
  - >-
    Let it cool in the pan for 10 minutes, then transfer to a wire rack. Try to
    wait at least 20 minutes before slicing. Try.
storage: >-
  Wrap tightly in plastic wrap or store in an airtight container at room
  temperature for up to 3 days. Refrigerate for up to a week. Slices freeze
  beautifully for up to 3 months — pop a frozen slice in the toaster for a warm
  treat.
leftover_ideas:
  - Toast a thick slice and spread with peanut butter and honey
  - Cube day-old bread and use it to make banana bread French toast
  - Crumble over vanilla ice cream for a banana bread sundae
nutrition:
  calories: 314
  protein: 4
  carbs: 47
  fat: 13
---
# Banana Bread

> Those overripe Costco bananas sitting on your counter aren't a failure — they're an opportunity. The browner, the better.

**Prep:** 15 min | **Cook:** 55 min | **Servings:** 10 | **Cost:** ~$0.75/serving

## Costco Shopping List
- Costco Bananas — 4 large, very ripe (the blacker the better) (~$0.60, 3 lb bunch)
- Kirkland All-Purpose Flour — 1 3/4 cups (~$0.30, 12 lb bag)
- Kirkland Unsalted Butter — 1/2 cup (1 stick), melted (~$0.50, 4-stick pack)
- Kirkland Large Eggs — 2 (~$0.50, 24-count)
- Kirkland Semi-Sweet Chocolate Chips — 1/2 cup, optional (~$0.75, 4.5 lb bag)

## Other Ingredients
- 3/4 cup granulated sugar
- 1 tsp pure vanilla extract
- 1 tsp baking soda
- 1/2 tsp salt
- 1/2 tsp ground cinnamon
- 1/4 cup sour cream or Greek yogurt

## Instructions
1. Preheat the oven to 350 degrees. Grease a 9x5-inch loaf pan or line it with parchment paper, leaving some overhang for easy removal.
2. Mash the bananas in a large bowl with a fork. You want them mostly smooth with a few chunks for texture. If your bananas aren't brown and spotted, they're not ready — put this recipe on hold and wait a couple days.
3. Stir the melted butter into the mashed bananas. Add the sugar, eggs, vanilla, and sour cream. Mix until just combined.
4. In a separate bowl, whisk together the flour, baking soda, salt, and cinnamon.
5. Fold the dry ingredients into the wet ingredients using a spatula. Mix until the flour just disappears — overmixing is the enemy of tender banana bread. A few small lumps are perfectly fine.
6. Fold in the chocolate chips if using. (You should be using them.)
7. Pour the batter into the prepared loaf pan and smooth the top. For that bakery-style split down the middle, run a butter knife lengthwise through the center of the batter.
8. Bake for 50–55 minutes, until a toothpick inserted in the center comes out with just a few moist crumbs. The top should be deeply golden and the edges pulling away from the pan.
9. Let it cool in the pan for 10 minutes, then transfer to a wire rack. Try to wait at least 20 minutes before slicing. Try.

> **Tip:** Freeze overripe bananas whole (peel and all) in a zip-top bag. When you've collected enough, thaw them and make bread. They'll be mushy and ugly — that's exactly what you want.

## Storage
Wrap tightly in plastic wrap or store in an airtight container at room temperature for up to 3 days. Refrigerate for up to a week. Slices freeze beautifully for up to 3 months — pop a frozen slice in the toaster for a warm treat.

## Leftover Ideas
- Toast a thick slice and spread with peanut butter and honey
- Cube day-old bread and use it to make banana bread French toast
- Crumble over vanilla ice cream for a banana bread sundae
